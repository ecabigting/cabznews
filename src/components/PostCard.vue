<template>
  <div class="bg-gray-800 rounded-lg p-4 flex flex-col sm:flex-row">
    <img
      v-if="post.image"
      :src="CreateURL(post.image, 111, 111)"
      alt="block w-full sm:max-w-xs mr-4 object-cover mb-4 sm:mb-0"
    />
    <div class="flex-1 flex flex-col">
      <h3 class="text-lg md:text-2xl font-bold mb-4 ml-5">{{ post.title }}</h3>
      <p class="text-gray-500 md:text-lg mb-4 flex-1">{{ post.exceprt }}</p>
      <div class="flex sm:flex-col md:flex-row justify-between items-start md:items-end">
        <div class="flex items-0center sm:mb-4 md:mb-0">
          <img
            ...
            :src="CreateURL(post.author.avatar, 100, 100)"
            alt=""
            class="rounded-full mr-4 w-12 h-12 ml-5"
          />
          <div class="">
